...
from nti.graphdb import _relationship as relationship
...
class TestFriendShips(ConfiguringTestBase):
	...
	def test_entity_friends(self):
		...
		result = relationship.entity_friends(owner)
		assert_that(result, has_length(3))
		for friend in (user1, user2, user3):
			key = relationship._Relationship(owner, friend)
			assert_that(key, is_in(result))

		result = relationship.entity_friends(user3)
	...
	def test_graph_friends(self):
		...
		result = relationship.graph_friends(self.db, user1)
		assert_that(result, has_length(2))

		user4 = self._create_random_user()
		self._create_friendslist(user1, "mycontacts2", user4)

		result = relationship.update_friendships(self.db, user1)
